...
class CrystalGraphConvNet(nn.Module):
    def __init__(self, orig_node_fea_len, edge_fea_len, h_fea_len=64,
                 node_fea_len=64, n_conv=3, n_h=2):
...
class PPO(nn.Module):
    def __init__(self, learning_rate=0.001, lmbda=0.95, gamma=1, alpha=0.5, beta=0.01, epsilon=0.2, discount_factor=1):
        ...
        self.gnn = CrystalGraphConvNet(4 + 15, 5, 64, self.node_fea_len, 3, 2)
        self.pi = MLP(self.node_fea_len + 10 + 5, 1).to(device)
    ...
    def calculate_pi(self, state_gnn, node_fea, edge_fea, edge_fea_idx, distance):
        # node_fea 9,13
        # edge_fea 9,3,5
        # edge_fea_idx 9, 3
        # distance 9,3
    ...
    def get_action(self, node_fea, edge_fea, edge_fea_idx, distance,type):
        with torch.no_grad():
            N, M = edge_fea_idx.shape
            # print(edge_fea_idx)
            # print(edge_fea)
            ...
            #print(state)
            probs = self.calculate_pi(state, node_fea, edge_fea, edge_fea_idx, distance)
            #print(probs) # type0 weight 작다
            mask = torch.where((edge_fea_idx >= 0) & (edge_fea[:,:,4]<=type), torch.tensor(0), torch.tensor(1)).unsqueeze(2)
    ...
    def update(self, data, probs, rewards, action, dones):
        num = 0
        ave_loss = 0
        en_loss = 0
        v_loss = 0
        ...
        for episode in data:
            for state in episode:
                ...
                if step < len(episode) - 1:
                    prob_a = self.calculate_pi(state_gnn,state[0], state[1], state[2], state[3])
                    mask = torch.where((state[2] >= 0) & (state[1][:,:,4]<=state[4]), torch.tensor(0), torch.tensor(1)).unsqueeze(2).to(device)
